...
type PollDailyReportMessage struct {
	...
	UserInformation *struct {
		...
		} `json:"polled" validate:"required"` // 用户参与了的投票数目（过去 24 小时）
...
type PollFinishedMessage struct {
	HitokotoBaseMessage
	PollID    int               `json:"id" validate:"required"`         // 投票 ID
	UpdatedAt *vcarbon.Carbon   `json:"updated_at" validate:"required"` // 投票更新时间，这里也是结束时间
	Username  string            `json:"user_name" validate:"required"`  // 审核员名字
	Status    consts.PollStatus `json:"status" validate:"required"`     // 投票结果： 200 入库，201 驳回，202 需要修改
	Method    consts.PollMethod `json:"method" validate:"required"`     // 审核员投票方式： 1 入库，2 驳回，3 需要修改
	Point     int               `json:"point" validate:"required"`      // 审核员投的票数
	// TODO: 加入审核员投票的意见标签？
}
